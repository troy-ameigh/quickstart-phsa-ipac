...
*A.* This Error could be due to Lambda Functions failing, this can be observed by accessing the cloudwatch and looking at the event. Reasons for failure could be due to input file not having the correct column names, and data, or date format. For more information, see link:#_data_schema_requirements[Data schema requirements].