...
Bulk patient data ingested by the deployment must be in a Microsoft Excel spreadsheet (.xlsx). The spreadsheet must contain three sheets, each containing the columns and data shown in the following lists. Items in bold are required. All others are optional but recommended.
