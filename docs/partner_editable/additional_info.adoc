...
* Use anonymous_patients.xlsx to test the workflow. This file is already uploaded to the assets folder in the S3 bucket deployed by the Quick Start. It is a spreadsheet containing fictitious patient data configured according to the link:#_data_schema_requirements[data schema requirements]. If the test is successful, labeling jobs are created in GroundTruth for reviewers.
